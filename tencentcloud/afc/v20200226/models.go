...
import (
    "encoding/json"
    tcerr "github.com/sesky4/tgo/tencentcloud/common/errors"
...
type AntiFraudVipRecord struct {
	...
	// 要时用到；
	// 注意：此字段可能返回 null，表示取不到有效值。
	OtherModelScores []*OtherModelScores `json:"OtherModelScores,omitempty" name:"OtherModelScores"`
...
type GetAntiFraudVipRequest struct {
	*tchttp.BaseRequest
	...
	// BusinessCryptoData)。
	BusinessSecurityData *AntiFraudVipFilter `json:"BusinessSecurityData,omitempty" name:"BusinessSecurityData"`

	// 默认不传，约定用密文业务
	// 入参(二选一
	// BusinessSecurityData 或
	// BusinessCryptoData)。
	BusinessCryptoData *AntiFraudVipCryptoFilter `json:"BusinessCryptoData,omitempty" name:"BusinessCryptoData"`
}
...
func (r *GetAntiFraudVipRequest) FromJsonString(s string) error {
	...
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError", "GetAntiFraudVipRequest has unknown keys!", "")
	}
...
type GetAntiFraudVipResponse struct {
	*tchttp.BaseResponse
	Response *struct {
		...
		// 反欺诈评分接口结果
		Data *AntiFraudVipRecord `json:"Data,omitempty" name:"Data"`
...
func (r *QueryAntiFraudVipRequest) FromJsonString(s string) error {
	...
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError", "QueryAntiFraudVipRequest has unknown keys!", "")
	}
	return json.Unmarshal([]byte(s), &r)
}
